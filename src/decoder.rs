...
impl SourceBlockDecoder {
    ...
    pub fn decode<T: IntoIterator<Item = EncodingPacket>>(
        ...
        // Case 2.5: low-loss fast path. When only a few source symbols are
        // missing, solve a reduced system over the lost symbols instead of
        // the full constraint matrix (see reduced.rs). Falls back silently.
        let lost_count = self.source_block_symbols - self.received_source_symbols;
        if lost_count > 0
            && lost_count <= crate::reduced::REDUCED_LOST_THRESHOLD
            && self.source_block_symbols >= crate::reduced::REDUCED_MIN_SYMBOLS
            && self.received_esi.len() >= self.source_block_symbols as usize
        {
            if let Some(result) = crate::reduced::try_reduced_decode(
                self.source_block_symbols,
                self.symbol_size,
                self.num_sub_blocks,
